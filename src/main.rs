...
fn main() {
    ...
    let to_run = get_day(2022, day_num);
    if let Ok(mut aoc) = Aoc::new()
        .year(Some(2021))
        .day(Some(day_num))
        .cache(Some(Path::new("./.aoc/cookie")))
        .init()
    {
        if let Ok(mut input) = aoc.get_input(true) {
...
fn run_funcs(to_run: (aoc::DayFn, aoc::DayFn), input: String) -> (Option<String>, Option<String>) {
